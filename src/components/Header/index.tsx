...
const Header = () => {
  ...
  const searchResults = useMemo(() => {
    ...
    return filterProducts(productList, searchQuery, selectedCategory !== "all" ? [selectedCategory] : []).slice(0, 6);
  ...
  return (
    ...
    >
      <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
        {/* <!-- header top start --> */}
        <div
          className="flex flex-col lg:flex-row gap-2 items-end lg:items-center xl:justify-between ease-out duration-200 py-0.5"
        >
          {/* <!-- header top left --> */}
          <div className="xl:w-auto flex-col sm:flex-row w-full flex sm:justify-between sm:items-center gap-5 sm:gap-10">
            <Link className="flex-shrink-0 block lg:-my-8 xl:-my-10 relative z-999" href="/">
              <img
                src="/logo-orange.svg"
                alt="NextTekno Logo"
                className="h-10 w-auto sm:h-20 lg:h-32 xl:h-44 transition-all duration-300 hover:scale-110 object-contain"
              />
            </Link>
              ...
              <div className="flex items-center gap-5">
                ...
                >
                  <span className="inline-block relative">
                    ...
                    <span className="flex items-center justify-center font-medium text-2xs absolute -right-2 -top-2.5 bg-orange-500 w-4.5 h-4.5 rounded-full text-white">
                      {product.length}
                    </span>
                  </span>
      ...
      <div className="border-t border-gray-3 py-0 h-10 flex items-center">
        <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
          <div className="flex items-center justify-between">
            {/* <!--=== Main Nav Start ===--> */}
            <div
              className={`w-[288px] absolute right-4 top-full xl:static xl:w-auto h-0 xl:h-auto invisible xl:visible xl:flex items-center justify-between xl:ml-12 ${navigationOpen &&
                ...
                <ul className="flex xl:items-center flex-col xl:flex-row gap-5 xl:gap-6">
                  {menuData.map((menuItem, i) =>
                    menuItem.title === "Mağaza" ? (
                      ...
                      >
                        <button
                          className={`hover:text-orange-500 text-custom-sm font-medium text-dark flex py-2 items-center gap-1`}
                        >
                          {menuItem.title}
                          <svg
                      ...
                      <li
                        key={i}
                        className="group relative before:w-0 before:h-[3px] before:bg-orange-500 before:absolute before:left-0 before:top-0 before:rounded-b-[3px] before:ease-out before:duration-200 hover:before:w-full "
                      >
                        <Link
                          href={menuItem.path}
                          className={`hover:text-orange-500 text-custom-sm font-medium text-dark flex py-2`
                          }
                        >
                          {menuItem.title}
                        </Link>
            ...
            {/* // <!--=== Nav Right Start ===--> */}
            {/* Removed "Son Baktıkların" and "Favorilerim" from here */}
            {/* // <!--=== Nav Right End ===--> */}
          </div>
        </div>
      </div>
